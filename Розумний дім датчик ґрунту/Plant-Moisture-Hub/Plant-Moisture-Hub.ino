...
// ==========================================
#define SOIL_PIN 3           // Аналоговий пін для Capacitive Soil Moisture Sensor v1.2 (ADC1)
...
const char* AP_SSID = "Plant-Soil-Hub";
const char* MDNS_NAME = "soilhub";
...
// Порогові значення вологості ґрунту для алертів (%)
float alert_soil_high = 80.0;
float alert_soil_low = 20.0;
const float HYSTERESIS_SOIL = 3.0;

// Калібрувальні значення ADC для датчика вологості ґрунту v1.2
// Увага: значення залежать від конкретного сенсора (потребують калібрування у сухому повітрі та у воді)
int soil_adc_air = 3200;   // Значення ADC у повністю сухому стані
int soil_adc_water = 1400; // Значення ADC повністю у воді
...
float soil_offset = 0.0;                    // Офсет калібрування відсотків (%)

unsigned long sensor_read_interval = 5000;  // Інтервал опитування датчика (мс)
...
// Кешовані показники
float cached_soil_moisture = NAN;
int cached_soil_adc = 0;

// Min/Max показники вологості
float min_soil = 999.0;
float max_soil = -999.0;
...
bool soil_high_alert_active = false;
bool soil_low_alert_active = false;
...
// Читання аналогового датчика вологості ґрунту v1.2
void readSensorData() {
  if (millis() - last_sensor_read >= sensor_read_interval || isnan(cached_soil_moisture)) {
    last_sensor_read = millis();
    
    // Зчитування середнього значення ADC (згладжування шуму)
    long sum_adc = 0;
    int samples = 10;
    for (int i = 0; i < samples; i++) {
      sum_adc += analogRead(SOIL_PIN);
      delay(5);
    }
    cached_soil_adc = sum_adc / samples;

    // Конвертація сирого значення ADC у відсотки вологості
    // Для ємнісного датчика: чим нижче значення ADC, тим вища вологість.
    float moisture = (float)(soil_adc_air - cached_soil_adc) / (soil_adc_air - soil_adc_water) * 100.0;
    moisture = constrain(moisture, 0.0, 100.0);

    cached_soil_moisture = moisture + soil_offset;
    cached_soil_moisture = constrain(cached_soil_moisture, 0.0, 100.0);

    if (cached_soil_moisture < min_soil) min_soil = cached_soil_moisture;
    if (cached_soil_moisture > max_soil) max_soil = cached_soil_moisture;
  }
}

// ==========================================
// 1. Головна сторінка налаштувань (Зелена тема для рослин)
// ==========================================
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    .section-title {
      font-size: 14px; font-weight: 800; color: #15803d; text-transform: uppercase;
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p class="subtitle">Панель керування v3.0 (Capacitive v1.2)</p>
    </div>
...
<head>
  ...
  <style>
    body { background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 50%, #f0fdf4 100%); background-attachment: fixed; color: #064e3b; font-family: system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; text-align: center; padding: 20px; }
    ...
    h2 { color: #15803d; font-size: 26px; font-weight: 800; margin-bottom: 12px; }
    p { color: #047857; font-size: 16px; font-weight: 600; }
  </style>
</head>
<body>
  <div class="card">
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 50%, #f0fdf4 100%); background-attachment: fixed; color: #064e3b; min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 24px; }
    ...
    .file-text { font-size: 15px; color: #047857; font-weight: 700; }
    .file-name { font-size: 14px; color: #15803d; margin-top: 8px; font-weight: 800; word-break: break-all; }
    ...
    .back-btn { display: inline-block; margin-top: 20px; color: #047857; text-decoration: none; font-size: 15px; font-weight: 700; }
  </style>
</head>
<body>
  <div class="card">
...
void loadConfig() {
  ...
  alert_soil_high = prefs.getFloat("sh", 80.0);
  alert_soil_low = prefs.getFloat("sl", 20.0);
  soil_adc_air = prefs.getInt("s_air", 3200);
  soil_adc_water = prefs.getInt("s_wat", 1400);
  ...
  soil_offset = prefs.getFloat("s_off", 0.0);
...
void saveSettings() {
  ...
  prefs.putFloat("sh", alert_soil_high);
  prefs.putFloat("sl", alert_soil_low);
  prefs.putInt("s_air", soil_adc_air);
  prefs.putInt("s_wat", soil_adc_water);
  ...
  prefs.putFloat("s_off", soil_offset);
...
String getMainMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌱 Вологість ґрунту\",\"callback_data\":\"status\"},{\"text\":\"📊 Статистика\",\"callback_data\":\"stats\"}],"
...
String getSettingsKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🚨 Алерти/Пороги\",\"callback_data\":\"alerts_menu\"},{\"text\":\"🎯 Калібрування ADC\",\"callback_data\":\"offsets_menu\"}],"
         "[{\"text\":\"⏱️ Інтервали\",\"callback_data\":\"intervals_menu\"},{\"text\":\"🌐 IP / Мережа\",\"callback_data\":\"ip_menu\"}],"
         "[{\"text\":\"🕒 NTP Час\",\"callback_data\":\"ntp_menu\"},{\"text\":\"📈 Екстремуми\",\"callback_data\":\"minmax\"}],"
         "[{\"text\":\"🔄 Скидання Eкстр.\",\"callback_data\":\"reset_minmax\"},{\"text\":\"📶 Wi-Fi Status\",\"callback_data\":\"wifi\"}],"
         "[{\"text\":\"⏱️ Uptime\",\"callback_data\":\"uptime\"},{\"text\":\"🏓 Ping\",\"callback_data\":\"ping\"}],"
...
String getAlertsMenuKeyboard() {
  ...
  return "{\"inline_keyboard\":["
         ...
         "[{\"text\":\"💧 High (" + String(alert_soil_high, 1) + "%)\",\"callback_data\":\"set_sh\"},"
         "{\"text\":\"🌵 Low (" + String(alert_soil_low, 1) + "%)\",\"callback_data\":\"set_sl\"}],"
...
String getOffsetsMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"☀️ ADC Air (" + String(soil_adc_air) + ")\",\"callback_data\":\"set_s_air\"},"
         "{\"text\":\"💧 ADC Water (" + String(soil_adc_water) + ")\",\"callback_data\":\"set_s_wat\"}],"
         "[{\"text\":\"🎯 Офсет (%) (" + String(soil_offset, 1) + ")\",\"callback_data\":\"set_s_off\"},"
         "{\"text\":\"🔙 Налаштування\",\"callback_data\":\"settings\"}]"
         "]}";
}
...
String getIntervalsMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌱 Сенсор (" + String(sensor_read_interval / 1000) + "с)\",\"callback_data\":\"set_s_int\"},"
...
void sendMenuMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "📋 <b>Головне меню — Soil Hub</b>\n\nОберіть потрібний розділ:", thread_id, getMainMenuKeyboard());
}

void sendSettingsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "⚙️ <b>Панель налаштувань та діагностики (Soil Hub)</b>", thread_id, getSettingsKeyboard());
}
...
void sendAlertsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  ...
  msg += "💧 Висока вологість: <code>" + String(alert_soil_high, 1) + " %</code>\n";
  msg += "🌵 Низька вологість: <code>" + String(alert_soil_low, 1) + " %</code>";
...
void sendOffsetsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "🎯 <b>Калібрування Capacitive Sensor v1.2</b>\n\n";
  msg += "☀️ ADC Air (сухе повітря): <code>" + String(soil_adc_air) + "</code>\n";
  msg += "💧 ADC Water (у воді): <code>" + String(soil_adc_water) + "</code>\n";
  msg += "🎯 Офсет вологості: <code>" + String(soil_offset, 1) + " %</code>";
...
void sendIntervalsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  ...
  msg += "🌱 Опитування сенсора: <code>" + String(sensor_read_interval / 1000) + " сек</code>\n";
...
void sendStatusMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "🌱 <b>Вологість ґрунту — Soil Hub</b>\n\n";
  if (isnan(cached_soil_moisture)) {
    msg += "⚠️ <i>Помилка зчитування датчика</i>";
  } else {
    msg += "<b>Вологість:</b> <code>" + String(cached_soil_moisture, 1) + " %</code>\n";
    msg += "<b>Сирий ADC:</b> <code>" + String(cached_soil_adc) + "</code>\n\n";
    if (cached_soil_moisture < alert_soil_low) msg += "🌵 <i>Ґрунт сухий! Потрібен полив.</i>";
    else if (cached_soil_moisture > alert_soil_high) msg += "💧 <i>Ґрунт занадто вологий.</i>";
    else msg += "🟢 <i>Вологість в нормі.</i>";
    msg += "\n\n🕒 <b>Час:</b> <code>" + getFormattedTime() + "</code>";
  }
...
void sendStatsMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "📊 <b>Статистика — Soil Hub</b>\n\n";
  if (!isnan(cached_soil_moisture)) {
    msg += "🌱 Вологість: <code>" + String(cached_soil_moisture, 1) + " %</code>\n";
    msg += "📊 Сирий ADC: <code>" + String(cached_soil_adc) + "</code>\n\n";
    msg += "📈 <b>Екстремуми вологості:</b>\n";
    msg += " ├ Мін: <code>" + String(min_soil, 1) + " %</code>\n";
    msg += " └ Макс: <code>" + String(max_soil, 1) + " %</code>\n\n";
  }
...
void sendMinMaxMessage(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "📈 <b>Екстремуми — Soil Hub</b>\n\n";
  msg += "🌱 Вологість ґрунту:\n";
  msg += " ├ Мін: <code>" + String(min_soil, 1) + " %</code>\n";
  msg += " └ Макс: <code>" + String(max_soil, 1) + " %</code>";
...
void sendPingMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "🏓 <b>Pong!</b> Soil Hub онлайн.\n🕒 <code>" + getFormattedTime() + "</code>", thread_id, getSubMenuKeyboard("ping"));
}
...
void processCallback(String data, String chat_id, String target_thread, long message_id) {
  ...
  else if (data == "set_sh") { waiting_input_chat_id = chat_id; waiting_input_param = "sh"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть порог високої вологості (%):", target_thread, ""); }
  else if (data == "set_sl") { waiting_input_chat_id = chat_id; waiting_input_param = "sl"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть порог низької вологості (%):", target_thread, ""); }
  else if (data == "set_s_air") { waiting_input_chat_id = chat_id; waiting_input_param = "s_air"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть значення ADC у сухому повітрі:", target_thread, ""); }
  else if (data == "set_s_wat") { waiting_input_chat_id = chat_id; waiting_input_param = "s_wat"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть значення ADC у воді:", target_thread, ""); }
  else if (data == "set_s_off") { waiting_input_chat_id = chat_id; waiting_input_param = "s_off"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть офсет вологості (%):", target_thread, ""); }
  else if (data == "set_s_int") { waiting_input_chat_id = chat_id; waiting_input_param = "s_int"; sendOrEditMessage(chat_id, message_id, "✏️ Інтервал сенсора (сек):", target_thread, ""); }
...
void processTextInput(String text, String chat_id, String target_thread) {
  if (waiting_input_chat_id == chat_id && waiting_input_param.length() > 0) {
    ...
    if (param == "sh") { alert_soil_high = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "sl") { alert_soil_low = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "s_air") { soil_adc_air = text.toInt(); saveSettings(); sendOffsetsMenu(chat_id, target_thread); }
    else if (param == "s_wat") { soil_adc_water = text.toInt(); saveSettings(); sendOffsetsMenu(chat_id, target_thread); }
    else if (param == "s_off") { soil_offset = text.toFloat(); saveSettings(); sendOffsetsMenu(chat_id, target_thread); }
...
// ==========================================
// Перевірка алертів вологості
// ==========================================
void checkSoilAlerts() {
  if (!alerts_enabled || ap_mode || WiFi.status() != WL_CONNECTED || strlen(bot_token) == 0) return;
  readSensorData();
  if (isnan(cached_soil_moisture)) return;
  ...
  // Сповіщення про сухий ґрунт
  if (cached_soil_moisture <= alert_soil_low && !soil_low_alert_active && can_send) {
    soil_low_alert_active = true; last_alert_time = millis();
    sendTelegramMessage(target_chat, "🌵 <b>Soil Hub: Ґрунт висохло! Потрібен полив!</b> <code>" + String(cached_soil_moisture, 1) + " %</code>", target_thread);
  } else if (cached_soil_moisture > (alert_soil_low + HYSTERESIS_SOIL) && soil_low_alert_active) {
    soil_low_alert_active = false;
    sendTelegramMessage(target_chat, "✅ <b>Soil Hub: Вологість ґрунту в нормі</b> <code>" + String(cached_soil_moisture, 1) + " %</code>", target_thread);
  }
}
    ...
    if (!boot_msg_sent && WiFi.status() == WL_CONNECTED) {
      ...
      if (strlen(bot_token) > 0) {
        ...
        if (target_chat.length() > 0) {
          sendTelegramMessage(target_chat, "🟢 <b>Soil Hub online (v3.0 Capacitive v1.2)</b>\nСистема готова до роботи.", target_thread, getMainMenuKeyboard());
        }
      }
    }
