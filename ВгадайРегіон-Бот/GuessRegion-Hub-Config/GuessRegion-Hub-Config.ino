...
#define GITHUB_REPO_NAME "guess_region_hub"
#define CURRENT_VERSION "3.0"

const byte DNS_PORT = 53;
const char* AP_SSID = "Region-Hub-AP";
const char* MDNS_NAME = "regionhub";
...
// ==========================================
// Системні змінні стану гри "Вгадай регіон"
// ==========================================
...
// База регіонів України
const int TOTAL_REGIONS = 5;
RegionData regionDatabase[TOTAL_REGIONS] = {
  {
    "Львівська область",
    "Львів",
    {"Івано-Франківськ", "Львів", "Тернопіль", "Луцьк"},
    1,
    "Культурна столиця західної України, відома своєю кавою та шоколадом."
  },
  {
    "Одеська область",
    "Одеса",
    {"Миколаїв", "Херсон", "Одеса", "Ізмаїл"},
    2,
    "Перлина біля моря, відома своєю архітектурою та гумором."
  },
  {
    "Харківська область",
    "Харків",
    {"Полтава", "Суми", "Дніпро", "Харків"},
    3,
    "Друге за розміром місто України, потужний науковий та промисловий центр."
  },
  {
    "Івано-Франківська область",
    "Івано-Франківськ",
    {"Івано-Франківськ", "Ужгород", "Чернівці", "Львів"},
    0,
    "Брама Карпат, колишній Станиславів."
  },
  {
    "Дніпропетровська область",
    "Дніпро",
    {"Запоріжжя", "Дніпро", "Кривий Ріг", "Полтава"},
    1,
    "Великий металургійний та космічний центр на річці Дніпро."
  }
};

// Стан активних ігрових сесій для користувачів [ChatID -> RegionIndex]
struct ActiveRegionSession {
  String chatId;
  int currentStepIndex;
  int score;
  bool active;
};
ActiveRegionSession activeSessions[5];

ActiveRegionSession* getSession(String chatId) {
  for (int i = 0; i < 5; i++) {
  ...
  for (int i = 0; i < 5; i++) {
    if (!activeSessions[i].active) {
      ...
      activeSessions[i].currentStepIndex = 0;
...
<head>
  ...
  <style>
    :root {
      ...
      --accent-hover: linear-gradient(135deg, #0284c7, #0369a1);
      --text-main: #1e293b;
      --text-sub: #475569;
    ...
    @media (prefers-color-scheme: dark) {
      :root {
        ...
        --accent-hover: linear-gradient(135deg, #0ea5e9, #0284c7);
        --text-main: #f8fafc;
        --text-sub: #cbd5e1;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    .section-title::after { content: ""; flex: 1; height: 2px; background: rgba(14, 165, 233, 0.2); }
    ...
    input:focus { border-color: var(--input-focus); box-shadow: 0 0 0 4px rgba(14, 165, 233, 0.18); }
    ...
    .btn-submit {
      ...
      box-shadow: 0 12px 24px rgba(14, 165, 233, 0.3); transition: all 0.2s ease;
    }
    .btn-submit:hover { background: var(--accent-hover); transform: translateY(-2px); box-shadow: 0 16px 32px rgba(14, 165, 233, 0.4); }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <h1>Guess Region Hub</h1>
      <p class="subtitle">Панель керування v3.0 (Telegram Вгадай Регіон)</p>
    </div>
...
<head>
  ...
  <style>
    ...
    h2 { color: #38bdf8; font-size: 26px; font-weight: 800; margin-bottom: 12px; }
...
<head>
  ...
  <style>
    ...
    .file-dropzone { position: relative; border: 2px dashed rgba(14, 165, 233, 0.4); border-radius: 22px; padding: 40px 20px; background: rgba(15, 23, 42, 0.6); cursor: pointer; margin-bottom: 28px; display: block; }
    ...
    .file-text { font-size: 15px; color: #38bdf8; font-weight: 700; }
    .file-name { font-size: 14px; color: #cbd5e1; margin-top: 8px; font-weight: 800; word-break: break-all; }
    button { width: 100%; padding: 18px; background: linear-gradient(135deg, #0ea5e9, #0284c7); border: none; border-radius: 18px; color: white; font-size: 17px; font-weight: 800; cursor: pointer; box-shadow: 0 12px 24px rgba(14, 165, 233, 0.3); }
    .back-btn { display: inline-block; margin-top: 20px; color: #38bdf8; text-decoration: none; font-size: 15px; font-weight: 700; }
  </style>
</head>
<body>
  <div class="card">
...
// ==========================================
// Конструктори Inline-клавіатур гри
// ==========================================
String getMainMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🗺️ Почати гру (Вгадай регіон)\",\"callback_data\":\"start_game\"}],"
...
String getRegionQuestionKeyboard(int rIndex) {
  RegionData r = regionDatabase[rIndex];
  String kb = "{\"inline_keyboard\":[";
  for (int i = 0; i < 4; i++) {
    kb += "[{\"text\":\"" + r.options[i] + "\",\"callback_data\":\"reg_" + String(rIndex) + "_" + String(i) + "\"}],";
  }
  kb += "[{\"text\":\"❌ Завершити гру\",\"callback_data\":\"end_game\"}]";
  kb += "]}";
  return kb;
}

// ==========================================
// Логіка гри "Вгадай регіон"
// ==========================================
void sendRegionQuestion(String chat_id, String thread_id, long message_id) {
  ActiveRegionSession* session = getSession(chat_id);
  if (session->currentStepIndex >= TOTAL_REGIONS) {
    // Гра завершена
    total_games_played++;
    String msg = "🎉 <b>Гру завершено!</b>\n\n";
    msg += "Ваш результат: <b>" + String(session->score) + " з " + String(TOTAL_REGIONS) + "</b> правильних відповідей.";
    endSession(chat_id);
    sendOrEditMessage(chat_id, message_id, msg, thread_id, getMainMenuKeyboard());
    return;
  }

  int rIdx = session->currentStepIndex;
  RegionData r = regionDatabase[rIdx];
  String msg = "🗺️ <b>Запитання " + String(rIdx + 1) + " з " + String(TOTAL_REGIONS) + "</b>\n\n";
  msg += "🔍 Якому регіону відповідає цей опис?\n<i>«" + r.hint + "»</i>";

  sendOrEditMessage(chat_id, message_id, msg, thread_id, getRegionQuestionKeyboard(rIdx));
}

void processRegionAnswer(String chat_id, String thread_id, long message_id, int rIdx, int chosenOpt) {
  ActiveRegionSession* session = getSession(chat_id);
  RegionData r = regionDatabase[rIdx];

  String msg = "";
  if (chosenOpt == r.correctIndex) {
    session->score++;
    total_correct_guesses++;
    msg = "✅ <b>Правильно!</b>\n\n";
  } else {
    total_wrong_guesses++;
    msg = "❌ <b>Неправильно.</b>\nПравильний регіон: <i>" + r.options[r.correctIndex] + "</i>\n\n";
  }
  msg += "📍 Обласний центр: <b>" + r.center + "</b>";

  // Відправляємо пояснення з кнопкою "Наступне"
  String nextKb = "{\"inline_keyboard\":[[{\"text\":\"➡️ Наступне запитання\",\"callback_data\":\"next_reg\"}]]}";
...
void sendMenuMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "📋 <b>Головне меню — Guess Region Hub</b>\n\nОберіть потрібний розділ:", thread_id, getMainMenuKeyboard());
}

void sendSettingsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "⚙️ <b>Панель налаштувань (Guess Region Hub)</b>", thread_id, getSettingsKeyboard());
}

void sendStatsMessage(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "📊 <b>Статистика гри — Guess Region Hub</b>\n\n";
  msg += "🎮 Зіграно ігор: <code>" + String(total_games_played) + "</code>\n";
  msg += "✅ Правильних відповідей: <code>" + String(total_correct_guesses) + "</code>\n";
  msg += "❌ Неправильних відповідей: <code>" + String(total_wrong_guesses) + "</code>\n\n";
...
void processCallback(String data, String chat_id, String target_thread, long message_id) {
  if (data == "menu") {
    ...
    ActiveRegionSession* s = getSession(chat_id);
    s->currentStepIndex = 0;
    s->score = 0;
    sendRegionQuestion(chat_id, target_thread, message_id);
  }
  else if (data == "next_reg") {
    sendRegionQuestion(chat_id, target_thread, message_id);
  }
  else if (data == "end_game") {
    endSession(chat_id);
    sendOrEditMessage(chat_id, message_id, "❌ Гру скасовано.", target_thread, getMainMenuKeyboard());
  }
  else if (data.startsWith("reg_")) {
    int firstUnderscore = data.indexOf('_');
    int secondUnderscore = data.lastIndexOf('_');
    int rIdx = data.substring(firstUnderscore + 1, secondUnderscore).toInt();
    int chosenOpt = data.substring(secondUnderscore + 1).toInt();
    processRegionAnswer(chat_id, target_thread, message_id, rIdx, chosenOpt);
  }
    ...
    if (!boot_msg_sent && WiFi.status() == WL_CONNECTED) {
      ...
      if (strlen(bot_token) > 0) {
        ...
        if (target_chat.length() > 0) {
          sendTelegramMessage(target_chat, "🟢 <b>Guess Region Hub online (v3.0)</b>\nГра \"Вгадай регіон\" готова до роботи.", target_thread, getMainMenuKeyboard());
        }
      }
    }
