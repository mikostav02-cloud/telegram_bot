...
#define GITHUB_REPO_NAME "weather_lite"
#define CURRENT_VERSION "3.0"

const byte DNS_PORT = 53;
const char* AP_SSID = "Weather-Info-Hub";
const char* MDNS_NAME = "weatherhub";
...
// OpenWeatherMap API конфігурація
char owm_api_key[45] = "";
char owm_city[45] = "Kyiv";
char owm_units[10] = "metric";
char owm_lang[5] = "ua";
...
unsigned long weather_fetch_interval = 600000; // Інтервал запиту погоди (мс) - 10 хв
unsigned long bot_interval = 2000;             // Інтервал перевірки Telegram (мс)
unsigned long alert_interval = 3600000;        // Інтервал повторних сповіщень (мс) - 1 год
...
// Кешовані показники погоди
float cached_temp = NAN;
float cached_feels_like = NAN;
int cached_humidity = -1;
float cached_pressure = 0;
float cached_wind_speed = 0;
int cached_wind_deg = 0;
String cached_description = "";
String cached_icon = "";

// Min/Max показники температури
...
// Отримання погоди з OpenWeatherMap API
bool fetchWeatherData() {
  if (strlen(owm_api_key) == 0 || WiFi.status() != WL_CONNECTED) return false;
  ...
  HTTPClient http;
  String url = "https://api.openweathermap.org/data/2.5/weather?q=" + String(owm_city) + "&units=" + String(owm_units) + "&lang=" + String(owm_lang) + "&appid=" + String(owm_api_key);
  ...
  if (http.begin(client, url)) {
    ...
    if (httpCode == HTTP_CODE_OK) {
      ...
      if (!error) {
        cached_temp = doc["main"]["temp"].as<float>();
        cached_feels_like = doc["main"]["feels_like"].as<float>();
        cached_humidity = doc["main"]["humidity"].as<int>();
        // Переведення тиску з гектопаскалей (hPa) в мм рт. ст.
        cached_pressure = doc["main"]["pressure"].as<float>() * 0.750062;
        cached_wind_speed = doc["wind"]["speed"].as<float>();
        cached_wind_deg = doc["wind"]["deg"].as<int>();
        cached_description = doc["weather"][0]["description"].as<String>();
        cached_icon = doc["weather"][0]["icon"].as<String>();
...
// ==========================================
// 1. Головна сторінка налаштувань (Блакитна тема для погоди)
// ==========================================
...
<head>
  ...
  <style>
    :root {
      ...
      --input-border: rgba(186, 230, 253, 0.8);
      --input-focus: #0284c7;
    }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <h1>Weather Info — Hub</h1>
      <p class="subtitle">Панель керування v3.0 (OpenWeatherMap)</p>
    </div>
    ...
    <form action="/save" method="POST">
      <div class="form-grid">
        ...
        <div class="full-width"><div class="section-title">OpenWeatherMap API</div></div>
        <div class="form-group">
          <label>API Key</label>
          <input type="password" name="owm_key" value="%OWM_KEY%" required>
        </div>
        <div class="form-group">
          <label>Місто (City)</label>
          <input type="text" name="owm_city" value="%OWM_CITY%" required>
        </div>
...
<head>
  ...
  <style>
    body { background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 50%, #f0f9ff 100%); background-attachment: fixed; color: #0c4a6e; font-family: system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; text-align: center; padding: 20px; }
    ...
    p { color: #0284c7; font-size: 16px; font-weight: 600; }
  </style>
</head>
<body>
  <div class="card">
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 50%, #f0f9ff 100%); background-attachment: fixed; color: #0c4a6e; min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 24px; }
    ...
    .file-text { font-size: 15px; color: #0284c7; font-weight: 700; }
    ...
    .back-btn { display: inline-block; margin-top: 20px; color: #0284c7; text-decoration: none; font-size: 15px; font-weight: 700; }
  </style>
</head>
<body>
  <div class="card">
...
void loadConfig() {
  ...
  String t_owm_key = prefs.getString("owm_key", "");
  String t_owm_city = prefs.getString("owm_city", "Kyiv");
  ...
  weather_fetch_interval = prefs.getULong("w_int", 600000);
  bot_interval = prefs.getULong("b_int", 2000);
  alert_interval = prefs.getULong("a_int", 3600000);
  ...
  t_owm_key = cleanString(t_owm_key);
  t_owm_city.trim();
  ...
  t_owm_key.toCharArray(owm_api_key, 45);
  t_owm_city.toCharArray(owm_city, 45);
}

void saveSettings() {
  prefs.begin("bot_config", false);
  prefs.putString("owm_key", owm_api_key);
  prefs.putString("owm_city", owm_city);
  ...
  prefs.putULong("w_int", weather_fetch_interval);
...
void saveConfig(String s, String p, String t, String u, String g, String th, String owm_k, String owm_c) {
  s = cleanString(s);
  p.trim();
  t = cleanString(t);
  u = cleanString(u);
  ...
  prefs.putString("owm_key", owm_k);
  prefs.putString("owm_city", owm_c);
  prefs.end();
  saveSettings();
}
...
void handleRoot() {
  ...
  page.replace("%OWM_KEY%", owm_api_key);
  page.replace("%OWM_CITY%", owm_city);
  server.send(200, "text/html", page);
}

void handleSave() {
  if (server.hasArg("ssid") && server.hasArg("token") && server.hasArg("owm_key")) {
    saveConfig(
      server.arg("ssid"),
      server.arg("pass"),
      server.arg("token"),
...
String getMainMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌤️ Погода зараз\",\"callback_data\":\"status\"},{\"text\":\"📊 Статистика\",\"callback_data\":\"stats\"}],"
...
String getSettingsKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🚨 Сповіщення\",\"callback_data\":\"alerts_menu\"},{\"text\":\"🌆 Місто / API\",\"callback_data\":\"city_menu\"}],"
...
String getCityMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌆 Місто: " + String(owm_city) + "\",\"callback_data\":\"set_owm_city\"}],"
         "[{\"text\":\"🔑 API Key (змінити)\",\"callback_data\":\"set_owm_key\"}],"
         "[{\"text\":\"🔙 Налаштування\",\"callback_data\":\"settings\"}]"
         "]}";
}
...
String getIntervalsMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌤️ Погода (" + String(weather_fetch_interval / 60000) + "хв)\",\"callback_data\":\"set_w_int\"},"
...
void sendMenuMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "📋 <b>Головне меню — Weather Hub</b>\n\nОберіть потрібний розділ:", thread_id, getMainMenuKeyboard());
}

void sendSettingsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "⚙️ <b>Панель налаштувань та діагностики (Weather Hub)</b>", thread_id, getSettingsKeyboard());
}
...
void sendCityMenu(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = String("🌆 <b>Налаштування міста та API</b>\n\n");
  msg += "🌆 Поточне місто: <code>" + String(owm_city) + "</code>\n";
  msg += "🔑 API Key: <code>" + String(strlen(owm_api_key) > 5 ? "налаштовано" : "відсутній") + "</code>";
  sendOrEditMessage(chat_id, message_id, msg, thread_id, getCityMenuKeyboard());
}

void sendIntervalsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "⏱️ <b>Системні інтервали</b>\n\n";
  msg += "🌤️ Запит погоди: <code>" + String(weather_fetch_interval / 60000) + " хв</code>\n";
...
void sendStatusMessage(String chat_id, String thread_id = "", long message_id = 0) {
  fetchWeatherData();
  String msg = "🌤️ <b>Погода у місті " + String(owm_city) + " — Weather Hub</b>\n\n";
  if (isnan(cached_temp)) {
    msg += "⚠️ <i>Помилка отримання даних погоди (перевірте API ключ)</i>";
  } else {
    msg += "<b> temperatura:</b> <code>" + String(cached_temp, 1) + " °C</code>\n";
    msg += "<b> Відчувається як:</b> <code>" + String(cached_feels_like, 1) + " °C</code>\n";
    msg += "<b> Опис:</b> <i>" + cached_description + "</i>\n";
    msg += "<b> Вологість:</b> <code>" + String(cached_humidity) + " %</code>\n";
    msg += "<b> Тиск:</b> <code>" + String(cached_pressure, 1) + " мм рт. ст.</code>\n";
    msg += "<b> Вітер:</b> <code>" + String(cached_wind_speed, 1) + " м/с (" + String(cached_wind_deg) + "°)</code>\n\n";
...
void sendStatsMessage(String chat_id, String thread_id = "", long message_id = 0) {
  fetchWeatherData();
  String msg = "📊 <b>Статистика погоди — Weather Hub</b>\n\n";
  if (!isnan(cached_temp)) {
    msg += "🌤️ Температура: <code>" + String(cached_temp, 1) + " °C</code>\n";
    msg += "💧 Вологість: <code>" + String(cached_humidity) + " %</code>\n\n";
    msg += "📈 <b>Екстремуми температури:</b>\n";
    msg += " ├ Мін: <code>" + String(min_temp, 1) + " °C</code>\n";
    msg += " └ Макс: <code>" + String(max_temp, 1) + " °C</code>\n\n";
  }
...
void sendMinMaxMessage(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "📈 <b>Екстремуми температури — Weather Hub</b>\n\n";
  msg += "🌤️ Місто " + String(owm_city) + ":\n";
  msg += " ├ Мін: <code>" + String(min_temp, 1) + " °C</code>\n";
  msg += " └ Макс: <code>" + String(max_temp, 1) + " °C</code>";
...
void sendResetMinMaxMessage(String chat_id, String thread_id = "", long message_id = 0) {
  ...
  sendOrEditMessage(chat_id, message_id, "🔄 <b>Екстремуми температури скинуто!</b>", thread_id, getSubMenuKeyboard("minmax"));
}
...
void sendPingMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "🏓 <b>Pong!</b> Weather Hub онлайн.\n🕒 <code>" + getFormattedTime() + "</code>", thread_id, getSubMenuKeyboard("ping"));
}
...
void processCallback(String data, String chat_id, String target_thread, long message_id) {
  ...
  else if (data == "city_menu") sendCityMenu(chat_id, target_thread, message_id);
  ...
  else if (data == "set_owm_city") { waiting_input_chat_id = chat_id; waiting_input_param = "owm_city"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть назву міста (наприклад, Kyiv):", target_thread, ""); }
  else if (data == "set_owm_key") { waiting_input_chat_id = chat_id; waiting_input_param = "owm_key"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть OpenWeatherMap API Key:", target_thread, ""); }
  else if (data == "set_w_int") { waiting_input_chat_id = chat_id; waiting_input_param = "w_int"; sendOrEditMessage(chat_id, message_id, "✏️ Інтервал запиту погоди (хв):", target_thread, ""); }
...
void processTextInput(String text, String chat_id, String target_thread) {
  if (waiting_input_chat_id == chat_id && waiting_input_param.length() > 0) {
    ...
    if (param == "owm_city") { text.toCharArray(owm_city, 45); saveSettings(); sendCityMenu(chat_id, target_thread); }
    else if (param == "owm_key") { text.toCharArray(owm_api_key, 45); saveSettings(); sendCityMenu(chat_id, target_thread); }
    else if (param == "w_int") { weather_fetch_interval = text.toInt() * 60000; saveSettings(); sendIntervalsMenu(chat_id, target_thread); }
  ...
  if (millis() - last_weather_fetch >= weather_fetch_interval) {
    if (fetchWeatherData()) {
      if (millis() - last_alert_time >= alert_interval || last_alert_time == 0) {
        ...
        String msg = "🌤️ <b>Weather Hub: Погода у місті " + String(owm_city) + "</b>\n\n";
        msg += "Температура: <code>" + String(cached_temp, 1) + " °C</code>\n";
        msg += "Опис: <i>" + cached_description + "</i>\n";
        msg += "Вологість: <code>" + String(cached_humidity) + " %</code>";
    ...
    if (!boot_msg_sent && WiFi.status() == WL_CONNECTED) {
      ...
      if (strlen(bot_token) > 0) {
        ...
        if (target_chat.length() > 0) {
          sendTelegramMessage(target_chat, "🟢 <b>Weather Hub online (v3.0 OpenWeatherMap)</b>\nСистема готова до роботи.", target_thread, getMainMenuKeyboard());
        }
      }
    }
