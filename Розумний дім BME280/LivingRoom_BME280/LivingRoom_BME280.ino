...
const char* AP_SSID = "LivingRoom-Climate-Hub";
const char* MDNS_NAME = "livingroom1";
...
char static_ip[16] = "192.168.1.201";
...
// ==========================================
// 1. Головна сторінка налаштувань
// ==========================================
...
<head>
  ...
  <title>Living Room — Sensor Hub</title>
  <style>
    :root {
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    .section-title {
      font-size: 14px; font-weight: 800; color: #059669; text-transform: uppercase;
      letter-spacing: 1.5px; margin: 16px 0 8px 0; display: flex; align-items: center; gap: 16px;
    }
    .section-title::after { content: ""; flex: 1; height: 2px; background: rgba(16, 185, 129, 0.15); }
    ...
    input:focus { border-color: var(--input-focus); background: #ffffff; box-shadow: 0 0 0 4px rgba(16, 185, 129, 0.18); }
    ...
    .btn-submit {
      ...
      box-shadow: 0 12px 24px rgba(16, 185, 129, 0.3); transition: all 0.2s ease;
    }
    .btn-submit:hover { background: var(--accent-hover); transform: translateY(-2px); box-shadow: 0 16px 32px rgba(16, 185, 129, 0.4); }
    ...
    .ota-link:hover { background: #ffffff; color: var(--text-main); border-color: rgba(16, 185, 129, 0.4); }
  </style>
</head>
<body>
  <div class="container">
...
<head>
  ...
  <style>
    body { background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 50%, #f0fdf4 100%); background-attachment: fixed; color: #0f172a; font-family: system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; text-align: center; padding: 20px; }
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 50%, #f0fdf4 100%); background-attachment: fixed; color: #0f172a; min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 24px; }
    ...
    .file-dropzone { position: relative; border: 2px dashed rgba(16,185,129,0.4); border-radius: 22px; padding: 40px 20px; background: rgba(255,255,255,0.6); cursor: pointer; margin-bottom: 28px; display: block; }
    ...
    .file-name { font-size: 14px; color: #059669; margin-top: 8px; font-weight: 800; word-break: break-all; }
    button { width: 100%; padding: 18px; background: linear-gradient(135deg, #10b981, #059669); border: none; border-radius: 18px; color: white; font-size: 17px; font-weight: 800; cursor: pointer; box-shadow: 0 12px 24px rgba(16,185,129,0.3); }
...
void sendMenuMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "📋 <b>Головне меню — Вітальня</b>\n\nОберіть потрібний розділ:", thread_id, getMainMenuKeyboard());
}

void sendSettingsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "⚙️ <b>Панель налаштувань та діагностики (Вітальня)</b>", thread_id, getSettingsKeyboard());
}
...
void sendStatusMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "🌡️ <b>Клімат — Вітальня</b>\n\n";
...
void sendPressureMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "🎈 <b>Атмосферний тиск — Вітальня</b>\n\n";
...
void sendDewPointMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "💧 <b>Точка роси — Вітальня</b>\n\n";
  if (isnan(cached_temp) || isnan(cached_hum)) {
    msg += "⚠️ <i>Помилка датчика</i>";
  } else {
...
void sendStatsMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "📊 <b>Статистика — Вітальня</b>\n\n";
...
void sendMinMaxMessage(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "📈 <b>Екстремуми — Вітальня</b>\n\n";
...
void sendPingMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "🏓 <b>Pong!</b> Вітальня онлайн.\n🕒 <code>" + getFormattedTime() + "</code>", thread_id, getSubMenuKeyboard("ping"));
}
...
void checkClimateAlerts() {
  ...
  if (cached_temp >= alert_temp_high && !temp_high_alert_active && can_send) {
    ...
    sendTelegramMessage(target_chat, "🚨 <b>Вітальня: Висока температура!</b> <code>" + String(cached_temp, 1) + " °C</code>", target_thread);
  } else if (cached_temp < (alert_temp_high - HYSTERESIS_TEMP) && temp_high_alert_active) {
    temp_high_alert_active = false;
    sendTelegramMessage(target_chat, "✅ <b>Вітальня: Температура в нормі</b> <code>" + String(cached_temp, 1) + " °C</code>", target_thread);
  }

  if (cached_press >= alert_press_high && !press_high_alert_active && can_send) {
    press_high_alert_active = true; last_alert_time = millis();
    sendTelegramMessage(target_chat, "📈 <b>Вітальня: Високий тиск!</b> <code>" + String(cached_press, 1) + " мм</code>", target_thread);
  } else if (cached_press < (alert_press_high - HYSTERESIS_PRESS) && press_high_alert_active) {
    press_high_alert_active = false;
    sendTelegramMessage(target_chat, "✅ <b>Вітальня: Тиск нормалізувався</b> <code>" + String(cached_press, 1) + " мм</code>", target_thread);
  }
}
    ...
    if (!boot_msg_sent && WiFi.status() == WL_CONNECTED) {
      ...
      if (strlen(bot_token) > 0) {
        ...
        if (target_chat.length() > 0) {
          sendTelegramMessage(target_chat, "🟢 <b>Вітальня online (v3.0 BME280)</b>\nСистема готова до роботи.", target_thread, getMainMenuKeyboard());
        }
      }
    }
