...
// ==========================================
#define BME_I2C_ADDRESS 0x76 // Або 0x77 залежно від модуля
...
const char* AP_SSID = "BedRoom2-Climate-Hub";
const char* MDNS_NAME = "bedroom2";

// ==========================================
// Глобальні об'єкти
// ==========================================
Adafruit_BME280 bme; // Використовуємо I2C (SDA=GPIO21, SCL=GPIO22 за замовчуванням на ESP32)
Preferences prefs;
DNSServer dnsServer;
WebServer server(80);
...
float alert_press_high = 780.0; // мм рт. ст.
float alert_press_low = 730.0;  // мм рт. ст.

const float HYSTERESIS_TEMP = 0.5;
const float HYSTERESIS_HUM = 2.0;
const float HYSTERESIS_PRESS = 3.0;
...
float press_offset = 0.0; // Офсет тиску (мм рт. ст.)

unsigned long sensor_read_interval = 5000;  // Інтервал опитування BME280 (мс)
...
// Показники датчика з урахуванням калібрування
float cached_temp = NAN;
float cached_hum = NAN;
float cached_press = NAN; // у мм рт. ст.
...
float min_hum = 999.0;
float max_hum = -999.0;
float min_press = 9999.0;
float max_press = -9999.0;
...
bool press_high_alert_active = false;
bool press_low_alert_active = false;
...
// Читання датчика BME280 з урахуванням калібрування
void readSensorData() {
  if (millis() - last_sensor_read >= sensor_read_interval || isnan(cached_temp)) {
    last_sensor_read = millis();
    float raw_t = bme.readTemperature();
    float raw_h = bme.readHumidity();
    float raw_p_hpa = bme.readPressure(); // Паскалі
    ...
    if (!isnan(raw_p_hpa)) {
      // Переведення гектопаскалів у мм рт. ст. (1 hPa = 0.750063755 mm Hg)
      cached_press = (raw_p_hpa * 0.00750063755) + press_offset;
      if (cached_press < min_press) min_press = cached_press;
      if (cached_press > max_press) max_press = cached_press;
    } else {
      cached_press = NAN;
    }
  }
...
// ==========================================
// 1. Головна сторінка налаштувань (Тема: Синя/Блакитна для Спальні 2)
// ==========================================
...
<head>
  ...
  <title>BedRoom 2 — Sensor Hub</title>
  <style>
    :root {
      ...
      --accent-hover: linear-gradient(135deg, #0369a1, #075985);
      --text-main: #0f172a;
      --text-sub: #475569;
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
    ...
    .section-title {
      font-size: 14px; font-weight: 800; color: #0369a1; text-transform: uppercase;
      letter-spacing: 1.5px; margin: 16px 0 8px 0; display: flex; align-items: center; gap: 16px;
    }
    .section-title::after { content: ""; flex: 1; height: 2px; background: rgba(2, 132, 199, 0.15); }
    ...
    input:focus { border-color: var(--input-focus); background: #ffffff; box-shadow: 0 0 0 4px rgba(2, 132, 199, 0.18); }
    ...
    .btn-submit {
      ...
      box-shadow: 0 12px 24px rgba(2, 132, 199, 0.3); transition: all 0.2s ease;
    }
    .btn-submit:hover { background: var(--accent-hover); transform: translateY(-2px); box-shadow: 0 16px 32px rgba(2, 132, 199, 0.4); }
    ...
    .ota-link:hover { background: #ffffff; color: var(--text-main); border-color: rgba(2, 132, 199, 0.4); }
  </style>
</head>
<body>
  <div class="container">
    <div class="header">
      ...
      <h1>Спальня 2 — Hub</h1>
      <p class="subtitle">Панель керування v3.0 (BME280)</p>
    </div>
...
<head>
  ...
  <style>
    body { background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 50%, #f0f9ff 100%); background-attachment: fixed; color: #0f172a; font-family: system-ui, sans-serif; display: flex; justify-content: center; align-items: center; min-height: 100vh; margin: 0; text-align: center; padding: 20px; }
    ...
    h2 { color: #0369a1; font-size: 26px; font-weight: 800; margin-bottom: 12px; }
...
<head>
  ...
  <style>
    body { font-family: system-ui, sans-serif; background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 50%, #f0f9ff 100%); background-attachment: fixed; color: #0f172a; min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 24px; }
    ...
    .file-dropzone { position: relative; border: 2px dashed rgba(2,132,199,0.4); border-radius: 22px; padding: 40px 20px; background: rgba(255,255,255,0.6); cursor: pointer; margin-bottom: 28px; display: block; }
    ...
    .file-text { font-size: 15px; color: #475569; font-weight: 700; }
    .file-name { font-size: 14px; color: #0369a1; margin-top: 8px; font-weight: 800; word-break: break-all; }
    button { width: 100%; padding: 18px; background: linear-gradient(135deg, #0284c7, #0369a1); border: none; border-radius: 18px; color: white; font-size: 17px; font-weight: 800; cursor: pointer; box-shadow: 0 12px 24px rgba(2,132,199,0.3); }
    .back-btn { display: inline-block; margin-top: 20px; color: #475569; text-decoration: none; font-size: 15px; font-weight: 700; }
  </style>
</head>
<body>
  <div class="card">
...
void loadConfig() {
  ...
  alert_press_high = prefs.getFloat("ph", 780.0);
  alert_press_low = prefs.getFloat("pl", 730.0);
  ...
  press_offset = prefs.getFloat("p_off", 0.0);
...
void saveSettings() {
  ...
  prefs.putFloat("ph", alert_press_high);
  prefs.putFloat("pl", alert_press_low);
...
String getMainMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌡️ Клімат\",\"callback_data\":\"status\"},{\"text\":\"🎈 Тиск\",\"callback_data\":\"pressure\"}],"
         "[{\"text\":\"💧 Точка роси\",\"callback_data\":\"dewpoint\"},{\"text\":\"⚙️ Налаштування\",\"callback_data\":\"settings\"}],"
...
String getAlertsMenuKeyboard() {
  ...
  return "{\"inline_keyboard\":["
         ...
         "[{\"text\":\"📈 High P (" + String(alert_press_high, 0) + ")\",\"callback_data\":\"set_ph\"},"
         "{\"text\":\"📉 Low P (" + String(alert_press_low, 0) + ")\",\"callback_data\":\"set_pl\"}],"
...
String getOffsetsMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌡️ T (" + String(temp_offset, 1) + "°C)\",\"callback_data\":\"set_toff\"},"
         "{\"text\":\"💧 H (" + String(hum_offset, 1) + "%)\",\"callback_data\":\"set_hoff\"}],"
         "[{\"text\":\"🎈 P (" + String(press_offset, 1) + "мм)\",\"callback_data\":\"set_poff\"},"
         "{\"text\":\"🔙 Налаштування\",\"callback_data\":\"settings\"}]"
         "]}";
}
...
String getIntervalsMenuKeyboard() {
  return "{\"inline_keyboard\":["
         "[{\"text\":\"🌡️ BME Poll (" + String(sensor_read_interval / 1000) + "с)\",\"callback_data\":\"set_s_int\"},"
...
void sendMenuMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "📋 <b>Головне меню — Спальня 2</b>\n\nОберіть потрібний розділ:", thread_id, getMainMenuKeyboard());
}

void sendSettingsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "⚙️ <b>Панель налаштувань та діагностики (Спальня 2)</b>", thread_id, getSettingsKeyboard());
}
...
void sendAlertsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  ...
  msg += "📈 Високий P: <code>" + String(alert_press_high, 0) + " мм</code>\n";
  msg += "📉 Низький P: <code>" + String(alert_press_low, 0) + " мм</code>";
...
void sendOffsetsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "🎯 <b>Калібрування BME280 (Офсети)</b>\n\n";
  msg += "🌡️ Офсет T: <code>" + String(temp_offset, 1) + " °C</code>\n";
  msg += "💧 Офсет H: <code>" + String(hum_offset, 1) + " %</code>\n";
  msg += "🎈 Офсет P: <code>" + String(press_offset, 1) + " мм рт. ст.</code>";
...
void sendIntervalsMenu(String chat_id, String thread_id = "", long message_id = 0) {
  ...
  msg += "🌡️ Опитування BME: <code>" + String(sensor_read_interval / 1000) + " сек</code>\n";
...
void sendPressureMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "🎈 <b>Атмосферний тиск — Спальня 2</b>\n\n";
  if (isnan(cached_press)) {
    msg += "⚠️ <i>Помилка датчика</i>";
  } else {
    msg += "<b>Поточний тиск:</b> <code>" + String(cached_press, 1) + " мм рт. ст.</code>\n\n";
    if (cached_press < 745) msg += "📉 <i>Низький тиск (дощ/негода)</i>";
    else if (cached_press <= 765) msg += "🟢 <i>Нормальний тиск</i>";
    else msg += "📈 <i>Підвищений тиск</i>";
  }
  String kb = "{\"inline_keyboard\":[[{\"text\":\"🔄 Оновити\",\"callback_data\":\"pressure\"},{\"text\":\"🔙 Меню\",\"callback_data\":\"menu\"}]]}";
  sendOrEditMessage(chat_id, message_id, msg, thread_id, kb);
}

void sendDewPointMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "💧 <b>Точка роси — Спальня 2</b>\n\n";
  if (isnan(cached_temp) || isnan(cached_hum)) {
    msg += "⚠️ <i>Помилка датчика</i>";
  } else {
...
void sendStatsMessage(String chat_id, String thread_id = "", long message_id = 0) {
  readSensorData();
  String msg = "📊 <b>Статистика — Спальня 2</b>\n\n";
  if (!isnan(cached_temp)) {
    msg += "🌡️ T: <code>" + String(cached_temp, 1) + " °C</code>\n";
    msg += "💧 H: <code>" + String(cached_hum, 1) + " %</code>\n";
    msg += "🎈 P: <code>" + String(cached_press, 1) + " мм</code>\n\n";
    msg += "📈 <b>Екстремуми:</b>\n";
    msg += " ├ T: <code>" + String(min_temp, 1) + " ... " + String(max_temp, 1) + " °C</code>\n";
    msg += " ├ H: <code>" + String(min_hum, 1) + " ... " + String(max_hum, 1) + " %</code>\n";
    msg += " └ P: <code>" + String(min_press, 1) + " ... " + String(max_press, 1) + " мм</code>\n\n";
  }
...
void sendMinMaxMessage(String chat_id, String thread_id = "", long message_id = 0) {
  String msg = "📈 <b>Екстремуми — Спальня 2</b>\n\n";
  msg += "🌡️ T: <code>" + String(min_temp, 1) + " ... " + String(max_temp, 1) + " °C</code>\n";
  msg += "💧 H: <code>" + String(min_hum, 1) + " ... " + String(max_hum, 1) + " %</code>\n";
  msg += "🎈 P: <code>" + String(min_press, 1) + " ... " + String(max_press, 1) + " мм</code>";
...
void sendResetMinMaxMessage(String chat_id, String thread_id = "", long message_id = 0) {
  ...
  min_hum = 999.0; max_hum = -999.0;
  min_press = 9999.0; max_press = -9999.0;
  sendOrEditMessage(chat_id, message_id, "🔄 <b>Екстремуми скинуто!</b>", thread_id, getSubMenuKeyboard("minmax"));
}
...
void sendPingMessage(String chat_id, String thread_id = "", long message_id = 0) {
  sendOrEditMessage(chat_id, message_id, "🏓 <b>Pong!</b> Спальня 2 онлайн.\n🕒 <code>" + getFormattedTime() + "</code>", thread_id, getSubMenuKeyboard("ping"));
}
...
void processCallback(String data, String chat_id, String target_thread, long message_id) {
  ...
  else if (data == "pressure") sendPressureMessage(chat_id, target_thread, message_id);
  ...
  else if (data == "set_th") { waiting_input_chat_id = chat_id; waiting_input_param = "th"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть порог високої температури (°C):", target_thread, ""); }
  else if (data == "set_tl") { waiting_input_chat_id = chat_id; waiting_input_param = "tl"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть порог низької температури (°C):", target_thread, ""); }
  else if (data == "set_hh") { waiting_input_chat_id = chat_id; waiting_input_param = "hh"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть порог високої вологості (%):", target_thread, ""); }
  else if (data == "set_hl") { waiting_input_chat_id = chat_id; waiting_input_param = "hl"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть порог низької вологості (%):", target_thread, ""); }
  else if (data == "set_ph") { waiting_input_chat_id = chat_id; waiting_input_param = "ph"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть високий тиск (мм):", target_thread, ""); }
  else if (data == "set_pl") { waiting_input_chat_id = chat_id; waiting_input_param = "pl"; sendOrEditMessage(chat_id, message_id, "✏️ Введіть низький тиск (мм):", target_thread, ""); }
  else if (data == "set_a_int") { waiting_input_chat_id = chat_id; waiting_input_param = "a_int"; sendOrEditMessage(chat_id, message_id, "✏️ Інтервал алертів (хв):", target_thread, ""); }
  else if (data == "set_toff") { waiting_input_chat_id = chat_id; waiting_input_param = "toff"; sendOrEditMessage(chat_id, message_id, "✏️ Офсет температури (°C):", target_thread, ""); }
  else if (data == "set_hoff") { waiting_input_chat_id = chat_id; waiting_input_param = "hoff"; sendOrEditMessage(chat_id, message_id, "✏️ Офсет вологості (%):", target_thread, ""); }
  else if (data == "set_poff") { waiting_input_chat_id = chat_id; waiting_input_param = "poff"; sendOrEditMessage(chat_id, message_id, "✏️ Офсет тиску (мм):", target_thread, ""); }
  else if (data == "set_s_int") { waiting_input_chat_id = chat_id; waiting_input_param = "s_int"; sendOrEditMessage(chat_id, message_id, "✏️ Інтервал датчика (сек):", target_thread, ""); }
...
void processTextInput(String text, String chat_id, String target_thread) {
  if (waiting_input_chat_id == chat_id && waiting_input_param.length() > 0) {
    ...
    if (param == "th") { alert_temp_high = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "tl") { alert_temp_low = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "hh") { alert_hum_high = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "hl") { alert_hum_low = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "ph") { alert_press_high = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "pl") { alert_press_low = text.toFloat(); saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "a_int") { alert_interval = text.toInt() * 60000; saveSettings(); sendAlertsMenu(chat_id, target_thread); }
    else if (param == "toff") { temp_offset = text.toFloat(); saveSettings(); sendOffsetsMenu(chat_id, target_thread); }
    else if (param == "hoff") { hum_offset = text.toFloat(); saveSettings(); sendOffsetsMenu(chat_id, target_thread); }
    else if (param == "poff") { press_offset = text.toFloat(); saveSettings(); sendOffsetsMenu(chat_id, target_thread); }
    else if (param == "s_int") { sensor_read_interval = text.toInt() * 1000; saveSettings(); sendIntervalsMenu(chat_id, target_thread); }
...
void checkClimateAlerts() {
  ...
  readSensorData();
  if (isnan(cached_temp) || isnan(cached_hum) || isnan(cached_press)) return;
  ...
  if (cached_temp >= alert_temp_high && !temp_high_alert_active && can_send) {
    temp_high_alert_active = true; last_alert_time = millis();
    sendTelegramMessage(target_chat, "🚨 <b>Спальня 2: Висока температура!</b> <code>" + String(cached_temp, 1) + " °C</code>", target_thread);
  } else if (cached_temp < (alert_temp_high - HYSTERESIS_TEMP) && temp_high_alert_active) {
    temp_high_alert_active = false;
    sendTelegramMessage(target_chat, "✅ <b>Спальня 2: Температура в нормі</b> <code>" + String(cached_temp, 1) + " °C</code>", target_thread);
  }

  if (cached_press >= alert_press_high && !press_high_alert_active && can_send) {
    press_high_alert_active = true; last_alert_time = millis();
    sendTelegramMessage(target_chat, "📈 <b>Спальня 2: Високий тиск!</b> <code>" + String(cached_press, 1) + " мм</code>", target_thread);
  } else if (cached_press < (alert_press_high - HYSTERESIS_PRESS) && press_high_alert_active) {
    press_high_alert_active = false;
    sendTelegramMessage(target_chat, "✅ <b>Спальня 2: Тиск нормалізувався</b> <code>" + String(cached_press, 1) + " мм</code>", target_thread);
  }
}
...
void setup() {
  ...
  // Ініціалізація BME280 на I2C
  if (!bme.begin(BME_I2C_ADDRESS)) {
    // Якщо не знайдено на 0x76, пробуємо 0x77
    bme.begin(0x77);
  }
    ...
    if (!boot_msg_sent && WiFi.status() == WL_CONNECTED) {
      ...
      if (strlen(bot_token) > 0) {
        ...
        if (target_chat.length() > 0) {
          sendTelegramMessage(target_chat, "🟢 <b>Спальня 2 online (v3.0 BME280)</b>\nСистема готова до роботи.", target_thread, getMainMenuKeyboard());
        }
      }
    }
